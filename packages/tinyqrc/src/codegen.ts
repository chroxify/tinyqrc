/**
 * Modified version of QR Code generator library by Project Nayuki
...
namespace qrcodegen {
  ...
  export class QrCode {
    ...
    public static encodeSegments(
      ...
      const dataCapacityBits: int =
        QrCode.getNumDataCodewords(version, finalEcl) * 8;
      ...
      return new QrCode(version, finalEcl, dataCodewords, mask);
    }
    ...
    public constructor(
      ...
      // Do masking
      let finalMask: int = msk;
      if (msk === -1) {
        // Automatically choose best mask
        let minPenalty: int = 1000000000;
        let bestMask: int = 0; // Default value
        for (let i = 0; i < 8; i++) {
          this.applyMask(i);
        ...
        finalMask = bestMask; // Use the best mask value
      }
      assert(0 <= finalMask && finalMask <= 7);
      this.mask = finalMask;
      this.applyMask(finalMask); // Apply the final choice of mask
      this.drawFormatBits(finalMask); // Overwrite old format bits
    ...
    private finderPenaltyTerminateAndCount(
      ...
      const extendedLength = currentRunLength + this.size;
      this.finderPenaltyAddHistory(extendedLength, runHistory);
      return this.finderPenaltyCountPatterns(runHistory);
    }
    ...
    private finderPenaltyAddHistory(
      ...
      const newRunLength =
        runHistory[0] === 0 ? currentRunLength + this.size : currentRunLength; // Add light border to initial run
      runHistory.pop();
      runHistory.unshift(newRunLength);
    }
  ...
  export class QrSegment {
    ...
    private static toUtf8ByteArray(str: string): Array<byte> {
      ...
      for (let i = 0; i < encoded.length; i++) {
        if (encoded.charAt(i) !== "%") result.push(encoded.charCodeAt(i));
        else {
          result.push(Number.parseInt(encoded.substr(i + 1, 2), 16));
          i += 2;
        }
      }
      return result;
